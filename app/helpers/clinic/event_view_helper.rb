module Clinic::EventViewHelper
	...
	def list_event_view(event)
		...
		r += link_to icon_tag(:check_circle), clinic_event_path(@clinic, event, event: {status: event.status == "報到" ? "已預約" : "報到"}), remote: true, method: "patch", class: "check_in #{"selected" if event.status == "報到"}"
		r += link_to icon_tag(:close_circle), clinic_event_path(@clinic, event, event: {status: event.status == "爽約" ? "已預約" : "爽約"}), remote: true, method: "patch", class: "break #{"selected" if event.status == "爽約"}"
		r += "</div>"
		r.html_safe
	end
